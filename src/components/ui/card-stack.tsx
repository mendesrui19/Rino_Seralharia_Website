            ...
            {items.map((item, i) => {
              ...
              return (
                <motion.div
                  key={item.id}
                  className={cn(
                    "absolute bottom-0 rounded-2xl border border-white/[0.06] bg-bg-sec overflow-hidden shadow-2xl",
                    "will-change-transform select-none",
                    isActive
                      ? "cursor-grab active:cursor-grabbing border-accent/30"
                      : "cursor-pointer",
                  )}
                  style={{
                    width: cardWidth,
        ...
        <div className="mt-8 flex items-center justify-center gap-3 relative z-20">
          ...
          {activeItem.href ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="text-white/50 hover:text-white transition ml-4"
